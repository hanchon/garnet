...
func DecodeDynamicField(schemaType SchemaType, encodingSlice []byte) interface{} {
	...
	default:
		...
		if staticSchemaType > 97 {
			logger.LogError(fmt.Sprintln("Unknown dynamic field type", zap.String("type", schemaType.String())))
			return ""
		}
		...
		if err != nil {
			logger.LogError(fmt.Sprintln("Could not marshal array", zap.Error(err)))
			return ""
		}
...
func DecodeStaticField(schemaType SchemaType, encoding []byte, bytesOffset uint64) interface{} {
	...
	if schemaType == STRING {
		...
		logger.LogError(fmt.Sprintln("Unknown static field type", zap.String("type", schemaType.String())))
		return ""
	}
}
